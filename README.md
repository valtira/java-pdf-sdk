...
This compiles the client into a jar located at `./target/PDFAPIClient-1.0-SNAPSHOT.jar`. Note that this jar does not include any dependencies.
...
```perl
dependencies {
    compile 'com.valtira.pdf.api:PDFAPIClient:1.0-SNAPSHOT'
}
```
...
<dependencies>
    <dependency>
        <groupId>com.valtira.pdf.api</groupId>
        <artifactId>PDFAPIClient</artifactId>
...
This will compile the client and package all dependencies into a jar located at `./target/PDFAPIClient-1.0-SNAPSHOT.jar`.
...
Service exceptions and client exceptions can be handled separately. Any exceptions modeled in the API will be a subtype of PDFAPIClientException.
```java
try {
    client.postAssemble(...);
} catch(PDFAPIClientException e) {
   // All service exceptions will extend from PDFAPIClientException.
   // Any unknown or unmodeled service exceptions will be represented as a PDFAPIClientException.
